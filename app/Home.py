...
def _build_architecture_figure() -> go.Figure:
    ...
    fig = go.Figure()
    # Wide enough gap, relative to box width, that neighboring two-line labels
    # never share horizontal space even at the narrowest supported viewport --
    # this figure is rendered full-width (not squeezed into a side column).
    box_w, gap = 1.0, 0.55
    for i, (label, subtitle) in enumerate(stages):
        ...
        fig.add_annotation(x=(x0 + x1) / 2, y=0.64, text=label.replace("\n", "<br>"), showarrow=False,
                            font=dict(size=12, color=INK_PRIMARY), align="center")
        fig.add_annotation(x=(x0 + x1) / 2, y=0.22, text=subtitle, showarrow=False,
                            font=dict(size=10, color=INK_MUTED), align="center")
        if i < len(stages) - 1:
            fig.add_annotation(
    ...
    fig.update_layout(
        height=380, showlegend=False, plot_bgcolor=SURFACE, paper_bgcolor=SURFACE,
        xaxis=dict(visible=False, range=[-0.2, (len(stages) - 1) * (box_w + gap) + box_w + 0.2], fixedrange=True),
